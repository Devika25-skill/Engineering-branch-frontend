...
export const usePdfDownloadDSY = () => {
  ...
  const generatePDF = async (recommendations: CollegeRecommendation[], formData: any) => {
    ...
    try {
      ...
      const addHeader = (pageNum: number = 1) => {
        ...
        pdf.text('College Recommendations Report - DSY Engineering', pageWidth - margin, headerYPos + 20, { align: 'right' });
      ...
      const sessionData = JSON.parse(sessionStorage.getItem('diplomaRecommendationFormData') || '{}');
      const userData = JSON.parse(localStorage.getItem('user') || '{}');
      const userName = userData.name || 'Student Name';
      const category = sessionData.reservationCategory || 'Not specified';
      const branches = sessionData.selectedBranches?.slice(0, 3)?.join(', ') || 'Not specified';
      const cetCutoff = sessionData.diplomaPercentage || 'Not specified';
      const locations = sessionData.selectedCities?.slice(0, 3)?.join(', ') || 'Not specified';
      
      // Display user details
      pdf.text(`Name: ${userName}`, margin + 5, yPosition);
      pdf.text(`Category: ${category}`, margin + 5, yPosition + 6);
      pdf.text(`Preferred Branches: ${branches}${!branches.includes('ALL') && formData.selectedBranches?.length > 3 ? ` (+${formData.selectedBranches.length - 3} streams)` : ''}`, margin + 5, yPosition + 12);
      pdf.text(`Preferred Locations: ${locations}${!locations.includes('ALL') && formData.selectedCities?.length > 3 ? ` (+${formData.selectedCities.length - 3} locations)` : ''}`, margin + 5, yPosition + 18);
      pdf.text(`Diploma Percentage: ${cetCutoff}`, margin + 5, yPosition + 24);
      ...
      sortedRecommendations.forEach((rec) => {
        ...
        if (rec.cutoff_percentile) {
          ...
          pdf.text(`${rec.cutoff_percentile}%`, pageWidth - margin - 25, yPosition + 12);
        }
      ...
      pdf.setFont('helvetica', 'normal');
      pdf.setTextColor(55, 65, 81);
      const disclaimerText = "These college recommendations are based on previous year's Round 1 cutoffs and your provided information. Please verify all details including current cutoffs, fees, course availability, and admission requirements before submitting your application forms.";
