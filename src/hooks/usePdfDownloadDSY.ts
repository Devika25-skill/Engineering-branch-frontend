  ...
  const generatePDF = async (
    ...
    try {
      ...
      const addHeader = (pageNum: number = 1) => {
        ...
        pdf.text(
          "College Recommendations Report - DSY Engineering",
          pageWidth - margin,
          headerYPos + 20,
          { align: "right" },
        );
      ...
      // Extract user details (prioritize passed formData)
      const sessionData = JSON.parse(
        sessionStorage.getItem("diplomaRecommendationFormData") || "{}",
      );
      // Merge session data with passed formData (formData takes precedence)
      const efficientData = { ...sessionData, ...formData };

      const userData = JSON.parse(localStorage.getItem("user") || "{}");
      const userName = userData.name || "Student Name";
      const category = efficientData.reservationCategory || "Not specified";
      const branches =
        efficientData.selectedBranches?.slice(0, 3)?.join(", ") ||
        "Not specified";
      const cetCutoff = efficientData.diplomaPercentage || "Not specified";
      const locations =
        efficientData.selectedCities?.slice(0, 3)?.join(", ") ||
        "Not specified";
      ...
      pdf.text(
        `Preferred Branches: ${branches}${!branches.includes("ALL") && formData.selectedBranches?.length > 3 ? ` (+${formData.selectedBranches.length - 3} streams)` : ""}`,
        margin + 5,
        yPosition + 12,
      );
      pdf.text(
        `Preferred Locations: ${locations}${!locations.includes("ALL") && formData.selectedCities?.length > 3 ? ` (+${formData.selectedCities.length - 3} locations)` : ""}`,
        margin + 5,
        yPosition + 18,
      );
      ...
      sortedRecommendations.forEach((rec) => {
        ...
        pdf.text(
          pdf.splitTextToSize(courseText, maxNameWidth)[0],
          margin + 15,
          yPosition + 14,
        );
      ...
      const disclaimerText =
        "These college recommendations are based on previous year's Round 1 cutoffs and your provided information. Please verify all details including current cutoffs, fees, course availability, and admission requirements before submitting your application forms.";
