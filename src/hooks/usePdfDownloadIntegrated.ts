...
export const usePdfDownload = () => {
  ...
  const generatePDF = async (recommendations: CollegeRecommendation[], formData: any) => {
    ...
    try {
      ...
      const branches = sessionDataDetails.branches?.slice(0, 3)?.join(', ') || 'Not specified';
      const cetCutoff = sessionData.score || 'Not specified';
      const locations = sessionDataDetails.cities?.slice(0, 3)?.join(', ') || 'Not specified';
      
      // Display user details
      pdf.text(`Name: ${userName}`, margin + 5, yPosition);
      pdf.text(`Category: ${category}`, margin + 5, yPosition + 6);
      pdf.text(`Preferred Branches: ${branches} ${sessionDataDetails.branches?.length > 3 ? `+${sessionDataDetails.branches?.length -3}` : ''}`, margin + 5, yPosition + 12);
      pdf.text(`Preferred Locations: ${locations} ${sessionDataDetails.cities?.length > 3 ? `+${sessionDataDetails.cities?.length -3}` : ''}`, margin + 5, yPosition + 18);
