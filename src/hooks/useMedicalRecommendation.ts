...
export const useMedicalRecommendation = () => {
  ...
  const generateMedicalRecommendation = async (formData: any) => {
    ...
    // Validate NEET Roll Number (10 digits) - Optional
    if (
    ...
    try {
      ...
      const roundNumber = (
        activeRound ? parseInt(activeRound.replace("round", ""), 10) : 1
      ) as 1 | 2 | 3;
      ...
      const response = await apiService.generateMedicalRecommendations(
      ...
      if (response.success) {
        ...
        const fullResponse = {
          ...
          accept_payment:
            response.data.accept_payment !== undefined
              ? response.data.accept_payment
              : true,
        };
