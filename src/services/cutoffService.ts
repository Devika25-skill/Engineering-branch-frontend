...
import { apiService, type RecommendationRequest, type RecommendationItem } from './api';
  ...
  async generateRecommendations(
    ...
    try {
      ...
      const payload: RecommendationRequest = {
        ...
        location: (formData.preferredCities || []).map((city: string) => city.toLowerCase())
      };
      ...
      ['Dream', 'Reach', 'Match', 'Safety'].forEach(category => {
        const categoryData = response.data[category as keyof typeof response.data] || [];
        ...
        categoryData.forEach((item: RecommendationItem) => {
          ...
          if (college.Overall_College_Placement_Percentage && college.Overall_College_Placement_Percentage > 70) {
            matchReasons.push('Good placement record');
          }

          if (college.College_Reviews_out_of_5 && college.College_Reviews_out_of_5 >= 4) {
          ...
          const recommendation: CollegeRecommendation = {
            college: {
              ...
              College_Bus_Facility_Available: college.College_Bus_Facility_Available,
