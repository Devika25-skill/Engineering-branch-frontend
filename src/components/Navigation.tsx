  ...
  useEffect(() => {
    const fetchTicketCount = async () => {
      if (isLoggedIn && user?.email && user?.accessToken) {
        try {
          const response = await ticketService.fetchUserTickets(
            user.email,
            user.accessToken,
          );
    ...
    <>
      ...
      <button
        onClick={() => {
          ...
          const savedRecommendationType = localStorage.getItem(
            "recommendation_type",
          );
          ...
          if (savedRecommendationType === "direct-second-year") {
            ...
            const hasExistingData = sessionStorage.getItem(
              `integrated_${savedIntegratedType}_formData`,
            );
            navigate(
              hasExistingData
                ? `/integrated-rounds?type=${savedIntegratedType}`
                : `/integrated-steps?type=${savedIntegratedType}`,
            );
          } else {
      ...
      >
        <div className="relative mr-2">
          ...
          <Sparkles
            size={12}
            className="absolute -top-1 -right-1 text-pink-400 opacity-0 group-hover:opacity-100 animate-pulse transition-opacity duration-300"
          />
        </div>
