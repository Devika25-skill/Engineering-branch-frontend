...
export const DiplomaAcademicInfoForm = ({
  data,
  onUpdate,
  validationErrors = [],
}: DiplomaAcademicInfoFormProps) => {
  ...
  useEffect(() => {
    ...
    if (!data.reservationCategory || !data.gender) {
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            <Label className="flex items-center gap-2 text-slate-700 font-semibold text-sm">
              ...
              <span className="text-red-500">*</span>
              {isFieldError("Last Year Diploma Percentage") && (
                <AlertCircle size={14} className="text-red-500" />
              )}
            </Label>
            <Input
              ...
              onChange={(e) =>
                handlePercentageChange("diplomaPercentage", e.target.value)
              }
              className={getFieldClassName(
                "Last Year Diploma Percentage",
