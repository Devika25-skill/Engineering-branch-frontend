...
export const Round3Tab = () => {
      ...
      if (user?.accessToken) {
        try {
          ...
          if (hasData) {
            ...
            // Don't auto-show preferences otherwise
          }
        } catch (error) {
  ...
  const handleCreateNewList = () => {
    ...
    // Persist active state for tab switching
    sessionStorage.setItem("round3CreateListActive", "true");

    // 2. Decide View Mode based on synchronous check of local storage/state
    const hasData =
      selectedBranches.length > 0 ||
      !!localStorage.getItem("round3Preferences");
  ...
  const handleRecommendationConfirmEdit = () => {
    ...
    // Keep selection state to allow re-generation with same college
  ...
  const handleFinalConfirm = async () => {
    // Clear "Create New List" persistent state
    sessionStorage.removeItem("round3CreateListActive");
    ...
    try {
      ...
      if (isKarnataka) {
        const payload = {
          round: 2, // Karnataka Round 3 uses round=2 for storage
          college_code:
  ...
  const handleUpdatePreferences = async () => {
    ...
    try {
      ...
      if (localStorage.getItem("selected_state") === "Karnataka") {
        ...
        await apiService.getUserRoundPreferences(3, user.accessToken);
  ...
  const handleGenerateRecommendations = async () => {
    ...
    // Validation removed: Round 2 college selection is optional.
    // If not selected, we proceed without it.
      ...
      if (isKarnataka) {
        ...
        if (response.success && response.data) {
          ...
          toast({
            ...
            description: "Your Round 3 recommendations have been generated.",
          });
        } else {
          toast({
        ...
        if (response.success && response.data) {
          ...
          setRound3Recommendations(convertedRecommendations);
          setHasGeneratedRecommendations(true);
        ...
        <>
          ...
          {!isConfirmed && (
            <div className="text-center">
              ...
              <p className="text-muted-foreground">
                Search and select the college you received in Round 2 for Round
                3 counselling
              </p>
            </div>
          )}
          ...
          >
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  Based on this selection, your Round 3 recommendation list will
                  be generated when you complete the preference settings. This
                  will help you find the best available options for your final
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  ...
                  will reset your current selection and any generated Round 3
          ...
          >
            <AlertDialogContent>
              <AlertDialogHeader>
                <AlertDialogTitle>Edit Selection?</AlertDialogTitle>
                <AlertDialogDescription>
                  Are you sure you want to edit your Round 3 Recommendation?
                  This will reset your current Round 3 Recommendation list and
                  any generated Round 3 Recommendation list will be affected.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogCancel>Cancel</AlertDialogCancel>
