...
export const IntegratedRound2Tab = ({ admissionType }: IntegratedRound2TabProps) => {
  ...
  useEffect(() => {
    ...
    const savedCollege = localStorage.getItem(`integrated_round2_college_${admissionType}`);
    const savedBranches = localStorage.getItem(`integrated_round2_branches_${admissionType}`);
    const savedCities = localStorage.getItem(`integrated_round2_cities_${admissionType}`);
  ...
  const handleCollegeSelect = (college: SelectedCollege) => {
    setSelectedCollege(college);
    localStorage.setItem(`integrated_round2_college_${admissionType}`, JSON.stringify(college));
    setCurrentStep('preferences');
  };
  ...
  const handleGenerateRecommendations = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        localStorage.setItem(`integrated_round2_branches_${admissionType}`, JSON.stringify(selectedBranches));
        localStorage.setItem(`integrated_round2_cities_${admissionType}`, JSON.stringify(selectedCities));
        setCurrentStep('results');
  ...
  const resetRound = () => {
    ...
    localStorage.removeItem(`integrated_round2_branches_${admissionType}`);
    localStorage.removeItem(`integrated_round2_cities_${admissionType}`);
