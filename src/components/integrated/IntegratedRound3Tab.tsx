...
export const IntegratedRound3Tab = ({ admissionType }: IntegratedRound3TabProps) => {
  ...
  const handleSubmitPreferences = async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        toast({
          title: "Success",
          description: "Round 3 recommendations generated successfully!",
        });
      } else {
  ...
  const filteredRecommendations = filteredCategory === 'All' 
    ...
    : round3Recommendations.filter(rec => rec.category === filteredCategory);

  const categoryStats = round3Recommendations.reduce((acc, rec) => {
          ...
          <CardContent>
            {!isUnlocked ? (
              <PremiumGate 
                ...
                productType="integrated_round_3"
                title="Unlock Integrated Round 3 Recommendations"
