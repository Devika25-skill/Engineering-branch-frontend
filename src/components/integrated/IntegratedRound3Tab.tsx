...
import { Search, ChevronDown, ChevronUp, Sparkles, Clock } from 'lucide-react';
...
export const IntegratedRound3Tab = ({ admissionType }: IntegratedRound3TabProps) => {
  ...
  const handleSubmitPreferences = async () => {
    ...
    try {
      ...
      toast({
        title: "Success",
        description: "Round 3 preferences saved successfully!",
      });
    } catch (error) {
      toast({
        title: "Error",
          ...
          <div className="text-center space-y-4">
            ...
            <div>
              ...
              <p className="text-yellow-700 text-sm max-w-md mx-auto">
                Final round for any remaining vacant seats. This is your last chance for seat allocation 
                and upgradation with immediate admission confirmation.
              </p>
            </div>
            <div className="flex items-center justify-center gap-2 text-sm text-yellow-600">
              <Sparkles className="w-4 h-4" />
              <span>Final opportunity for your dream college!</span>
            </div>
          </div>
        </CardContent>
      </Card>
